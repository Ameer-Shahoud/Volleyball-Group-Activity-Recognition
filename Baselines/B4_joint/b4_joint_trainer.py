...
from Baselines.B4_joint.b4_joint_model import B4JointModel
...
class B4JointTrainer(_BaseTrainer):
    ...
    def _get_model(self) -> B4JointModel:
        return B4JointModel()
    ...
    def get_optimizers(self) -> list[torch.optim.Optimizer]:
        model: B4JointModel = self._model
        return [optim.Adam(
            (
                p for p in chain(
                    model.player_lstm.parameters(),
                    model.player_base.parameters()
                ) if p.requires_grad
            ),
